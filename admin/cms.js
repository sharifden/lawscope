(() => {
  ...
  function browserServiceUrl(origin, path) {
    return `${origin}${path}`;
  }
  ...
  if (companionOrigin) {
    ...
    const identityUrl = browserServiceUrl(serviceOrigin, '/.netlify/identity');
    const gatewayUrl = browserServiceUrl(serviceOrigin, '/.netlify/git/github');
