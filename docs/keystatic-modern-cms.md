...
- Production `/dashboard` login uses same-origin Identity and Git Gateway paths that Vercel proxies to the Decap companion. Local `npm run build` still says “fail-closed until CMS_COMPANION_ORIGIN is provisioned” unless you set that variable or `VERCEL_ENV=production`. Keystatic does not need the companion.
