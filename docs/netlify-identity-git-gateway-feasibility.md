...
- A single validated `CMS_COMPANION_ORIGIN` build variable still names the upstream companion; production browsers call native same-origin `/.netlify/identity` and `/.netlify/git` URLs, which Vercel rewrites onto the path-limited `/api/cms-gateway` function instead of calling that origin directly.
