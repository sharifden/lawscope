...
3. Production browsers call the native same-origin `/.netlify/identity` and `/.netlify/git` paths that Netlify Identity Widget 2.0.3 and Decap CMS already construct (`APIUrl + '/settings'`, `gatewayUrl + '/settings'`, and later `/token`, `/user`, `/github/...`). Vercel rewrites those routes onto the path-limited `/api/cms-gateway` proxy, including static Identity/Git settings destinations so trailing-slash handling cannot fall through to the public 404 page. The proxy forwards only approved upstream prefixes, copies the browser `Authorization` header when present, and never injects a GitHub, Netlify, or repository token. Local and Preview stay fail-closed unless `CMS_COMPANION_ORIGIN` is set.
...
- `admin/cms-identity-init.js` initializes the Identity widget with the native `/.netlify/identity` API root before Decap can call `init()` without an APIUrl.
...
6. Verify production browser requests stay on `https://getlawscope.com/.netlify/identity` and `https://getlawscope.com/.netlify/git`, including the widget’s `/.netlify/identity/settings` probe, that the gateway forwards them to the companion, and that no token appears in query strings, referrers, logs, analytics, or error reporting.
